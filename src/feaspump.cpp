...
static double solutionsLinfDistance(const std::vector<int> &integers, const std::vector<double> &x1, const std::vector<double> &x2)
{
	double dist = 0.0;
	for (int j : integers)
		dist = std::max(dist, fabs(x1[j] - x2[j]));
	return dist;
}
...
namespace dominiqs
{
	...
	static double DEF_IMPR_THREASHOLD = 0.9;
	...
	FeasibilityPump::FeasibilityPump() : timeLimit(DEF_TIME_LIMIT) /*, timeMult(DEF_TIME_MULT)*/, lpIterMult(DEF_LPITER_MULT),
										 ...
										 objOffset(0.0), hasIncumbent(false), numIntegersObj(DEF_INTEGERS_OBJ), mipPresolve(true),
										 forceNullObjectiveInitialLP(false), forceSumVarsObjectiveInitialLP(false),
										 reverseObjectiveFunction(false),
										 forceSumVarsObjectiveInitialLPMaxSense(false), numFracsObj(DEF_FRACS_OBJ),
										 newScaleC(DEF_NEW_OBJ_SCALE), newScaleDelta(DEF_SCALE_C_AND_DELTA), stage1NoImprIterLimit(ITR1_NO_IMPR),
										 stage2NoImprIterLimit(ITR2_NO_IMPR), imprThreshold(DEF_IMPR_THREASHOLD), fracScaleFactor(DEF_FRAC_SCALE_FACTOR), intScaleFactor(DEF_INTEGER_SCALE_OBJ),
	...
	void FeasibilityPump::readConfig()
	{
		...
		if (firstMethod == "default")
			firstOptMethod = 'S';
		else if (firstMethod == "primal")
			firstOptMethod = 'P';
		else if (firstMethod == "dual")
			firstOptMethod = 'D';
		else if (firstMethod == "barrier")
			firstOptMethod = 'B';
		else
			throw std::runtime_error(std::string("Unknown optimization method: ") + firstMethod);
		std::string reMethod = gConfig().get("fp.reOptMethod", std::string("default"));
		if (reMethod == "default")
			reOptMethod = 'S';
		else if (reMethod == "primal")
			reOptMethod = 'P';
		else if (reMethod == "dual")
			reOptMethod = 'D';
		else if (reMethod == "barrier")
			reOptMethod = 'B';
		else
			throw std::runtime_error(std::string("Unknown optimization method: ") + reMethod);
		// other options
		timeLimit = gConfig().get("timeLimit", 1e+20);
		...
		READ_FROM_CONFIG(stage1NoImprIterLimit, ITR1_NO_IMPR);
		READ_FROM_CONFIG(stage2NoImprIterLimit, ITR2_NO_IMPR);
		READ_FROM_CONFIG(iterLimit, DEF_ITER_LIMIT);
		READ_FROM_CONFIG(imprThreshold, DEF_IMPR_THREASHOLD);
		...
		READ_FROM_CONFIG(forceNullObjectiveInitialLP, false);
		READ_FROM_CONFIG(forceSumVarsObjectiveInitialLP, false);
		READ_FROM_CONFIG(reverseObjectiveFunction, false);
		READ_FROM_CONFIG(forceSumVarsObjectiveInitialLPMaxSense, false);

		// new options
		mipPresolve = gConfig().get("mipPresolve", true);
		...
		LOG_CONFIG(stage1NoImprIterLimit);
		LOG_CONFIG(stage2NoImprIterLimit);
		LOG_CONFIG(imprThreshold);
		LOG_CONFIG(avgFlips);
		...
		LOG_CONFIG(forceNullObjectiveInitialLP);
		LOG_CONFIG(forceSumVarsObjectiveInitialLP);
		LOG_CONFIG(forceSumVarsObjectiveInitialLPMaxSense);
		LOG_CONFIG(reverseObjectiveFunction);
	...
	void FeasibilityPump::getClosestFrac(std::vector<double> &x) const
	{
		if (closestFrac.empty())
		{
			x.clear();
			return;
		}
		...
		if (hasPresolve)
		{
			DOMINIQS_ASSERT((int)closestFrac.size() == model->ncols());
			postClosestFrac = originalModel->postsolveSolution(closestFrac);
		}
		else
			postClosestFrac = closestFrac;
	...
	void FeasibilityPump::getSolution(std::vector<double> &x) const
	{
		...
		// uncrush solution
		std::vector<double> postIncumbent;
		if (hasPresolve)
		{
			DOMINIQS_ASSERT((int)incumbent.size() == model->ncols());
			// consoleError("{} == {} != {}", incumbent.size(), model->ncols(), originalModel->ncols());
			postIncumbent = originalModel->postsolveSolution(incumbent);
		}
		else
			postIncumbent = incumbent;

		x.resize(postIncumbent.size());
		copy(postIncumbent.begin(), postIncumbent.end(), x.begin());
	}
	...
	void FeasibilityPump::resetPartial()
	{
		hasPresolve = false;
		lastIntegerX.clear();
	...
	void FeasibilityPump::resetTotal()
	{
		resetPartial();
		nitr = 0;
		firstPerturbation = 0;
	...
	bool FeasibilityPump::init(MIPModelPtr _model, const std::vector<char> &ctype)
	{
		...
		originalModel = _model; // must be saved for converting post solve solution in case of presolve.

		consoleLog("fpProblem: #rows={} #cols={} #nnz={}",
				   originalModel->nrows(), originalModel->ncols(), originalModel->nnz());
		// presolve
		MIPModelPtr premodel;
		if (mipPresolve)
		{
			originalModel->dblParam(DblParam::TimeLimit, timeLimit);
			if (!(originalModel->presolve())) // if fails, already halts, cause problem is infeasible!
			{
				consoleLog("fpPresolvedProblem: MIP infeasible");
				return false;
			}
			premodel = originalModel->presolvedModel();
			if (!premodel)
			{
				// presolve made no reduction: just clone the original model
				consoleLog("fpPresolvedProblem: no reductions");
				premodel = originalModel->clone();
			}
			else
			{
				if (premodel->nrows() > 0)
				{
					hasPresolve = true;
					consoleLog("fpPresolvedProblem: #rows={} #cols={} #nnz={}",
							   premodel->nrows(), premodel->ncols(), premodel->nnz());
				}
				else //  in case presolved model has no constraints, undo presolve.
				{
		...
		for (int i = 0; i < n; i++)
		{
			...
			if (equal(lb[i], ub[i], integralityEps))
				fixed.push_back(i);
			else if (xType[i] != 'C')
			{
		...
		// model->colNames(varNames);

		// for (int i = 0; i < rows.size(); ++i)
		// {
		// 	(rows[i])->print(varNames);
		// }

		isBinary = (gintegers.size() == 0);
		isPureInteger = (fixed.size() + integers.size() == (unsigned int)n);
		consoleLog("#cols = {} #bins = {} #integers = {} #continuous = {}", n, binaries.size(), gintegers.size(), num_continuous_vars);
	...
	std::tuple<bool, bool> FeasibilityPump::pump(double time_limit, bool stopWithNoImprLimit, const std::vector<double> &xStartFrac, double xStartDist, bool pFeas)
	{
		...
		LOG_CONFIG(stage2NoImprIterLimit);
		LOG_CONFIG(imprThreshold);
		LOG_CONFIG(stopWithNoImprLimit);
		LOG_CONFIG(forceNullObjectiveInitialLP);
		LOG_CONFIG(forceSumVarsObjectiveInitialLP);
		LOG_CONFIG(forceSumVarsObjectiveInitialLPMaxSense);
		LOG_CONFIG(reverseObjectiveFunction);
		...
		LOG_ITEM("fp.firstOptMethod", firstMethod);
		LOG_ITEM("fp.reOptMethod", reMethod);
		consoleLog("fp.timeLimit = {:.2f}", timeLimit);
		...
		if (!xStartFrac.empty())
		{
			// IMPORTANT: if presolved in current call, must map the given starting point to the presolved model.
			std::vector<double> presolvedxStartFrac;
			if (hasPresolve)
			{
				DOMINIQS_ASSERT((int)xStartFrac.size() == originalModel->ncols());
				presolvedxStartFrac = originalModel->presolveSolution(xStartFrac);
			}
			else
				presolvedxStartFrac = xStartFrac;

			frac_x = presolvedxStartFrac;
			DOMINIQS_ASSERT((int)frac_x.size() == n);
			closestFrac = presolvedxStartFrac;
			// closestPoint = presolvedxStartFrac;
			primalFeas = pFeas;
			consoleInfo("[initialSolve skipped]");
			consoleLog("initial frac starting point given with previous closestDist = {:.4f}", xStartDist);
			closestDist = INFBOUND;
			consoleWarn("closestDist reset to {}", closestDist);
		}
		else
		{
			closestDist = INFBOUND;
			solveInitialLP();
			if (primalFeas)
				dualBound = getSolutionValue(frac_x);

			if (model->isInfeasibleOrTimeReached())
			{
				consoleLog("Infeasible or time reached");
				return {false, false}; // returns false if LP already infeasible.
			}
		}
		...
		if (runningAlpha == 0.0)
			display.setVisible("alpha", false);

		display.printHeader(std::cout);

		int stage = 1;
		// stage 1
		bool found = pumpLoop(runningAlpha, stage, dualBound, stopWithNoImprLimit);
		...
		if (!found || gintegers.size())
		{
			if (closestPoint.empty())
				closestDist = INFBOUND;
			else if (gintegers.size())
			{
				// update value of ClosestDist based on the total set of integer variables (cause, in stage1, only considers the binary variables in the distance computation).
				frac2int->ignoreGeneralIntegers(false);
				frac2int->apply(closestFrac, closestPoint);
				closestDist = solutionsDistance(integers, closestFrac, closestPoint);
				// consoleInfo("ClosestDist: {}", closestDist);
			}

			stage = 2;
			found = pumpLoop(runningAlpha, stage, dualBound, stopWithNoImprLimit);
		}
		consoleLog("");
		...
		if (found)
			foundIncumbent(frac_x, getSolutionValue(frac_x));
		...
		if (stage == 3)
			LOG_ITEM("stage3Time", stage3Time);
		return {found, !closestFrac.empty()};
	}
	...
	void FeasibilityPump::solveInitialLP()
	{
		...
		if (forceNullObjectiveInitialLP)
		{
			...
			model->objcoefs(num_vars, &(indexes[0]), &(coefs[0]));
			model->objOffset(0.0); //< get rid of offset
		}
		else if (forceSumVarsObjectiveInitialLP)
		{
			int num_vars = model->ncols();
			std::vector<double> coefs(num_vars, 0);
			std::vector<int> indexes(num_vars);
			...
			model->objcoefs(num_vars, &(indexes[0]), &(coefs[0]));
			if (forceSumVarsObjectiveInitialLPMaxSense)
				model->objSense(ObjSense::MAX);
			else
				model->objSense(ObjSense::MIN);
			model->objOffset(0.0); //< get rid of offset
		}

		// model->writeModel("antes.lp");
		if (reverseObjectiveFunction)
		{
			const auto curr_obj_sense = model->objSense();
			model->objSense(curr_obj_sense == ObjSense::MAX ? ObjSense::MIN : ObjSense::MAX);
		}

		// model->writeModel("depois.lp");
		...
		// model->lbs(&test_lbs[0]);
		// model->ubs(&test_ubs[0]);
		// model->ctypes(&test_types[0]);
		...
		// size_t test_num_rows = model->nrows();
		// for (int i = 0; i < n; i++)
		// {
		// 	// consoleLog("var {}",i);
		// 	// fixed variables are not considered integer variables, of any kind
		...
		// consoleInfo("model before pump: fixed 0: {} | fixed 1: {} | integer: {} | binary: {} | continuous: {} | rows: {}", test_num_fixed_zero_vars, test_num_fixed_one_vars, test_num_integer_vars, test_num_binary_vars, test_num_continuous_vars, test_num_rows);
		// model->switchToLP();

		model->dblParam(DblParam::TimeLimit, timeLeft);
		// StopWatch tp1;
		// tp1.start();
		bool result = model->lpopt(firstOptMethod, false, true);
		// if (greaterThan(tp1.getElapsed(), timeLeft))
		// 	std::cout << " * " << tp1.getElapsed() << " > " << timeLeft << std::endl;
		...
		if (!result || model->aborted()) // result == false means LP is infeasible!
			return;

		rootLpIter = std::max(simplexIt, barrierIt);
		if (model->isPrimalFeas())
		{
			// consoleInfo("is primal feasible");
			model->sol(&frac_x[0]);
			if (model->isSolutionFeasible(frac_x))
			{
				primalFeas = true;
				// consoleInfo("solution is feasible");
			}
		}
	...
	void FeasibilityPump::perturbe(std::vector<double> &x, bool ignoreGeneralIntegers)
	{
		...
		if (ignoreGeneralIntegers)
		{
			for (int j : binaries)
			{
				...
				if (greaterThan(sigma, integralityEps))
					toOrder.emplace(sigma, j);
			}
		}
		else
		{
		...
		while ((itr != end) && (flipsDone < nflips))
		{
			...
			if (equal(x[toFlip], lb[toFlip], integralityEps))
			{
				x[toFlip] += 1.0;
				++flipsDone;
			}
			else if (equal(x[toFlip], ub[toFlip], integralityEps))
			{
				x[toFlip] -= 1.0;
				++flipsDone;
			}
			else
			{
				...
				if (lessThan(x[toFlip], frac_x[toFlip], integralityEps))
				{
					x[toFlip] += 1.0;
					++flipsDone;
				}
				if (greaterThan(x[toFlip], frac_x[toFlip], integralityEps))
				{
					x[toFlip] -= 1.0;
					++flipsDone;
				}
	...
	void FeasibilityPump::restart(std::vector<double> &x, bool ignoreGeneralIntegers)
	{
		...
		for (unsigned int i = 0; i < size; i++)
		{
			int j = binaries[i];
			r = rnd.getFloat() - 0.47;
			if (greaterThan(r, 0) && equal(x[j], previousSol[j], integralityEps))
			{
				sigma = fabs(x[j] - frac_x[j]);
				if (greaterThan(sigma + r, 0.5))
				{
		...
		if (!ignoreGeneralIntegers && gintegers.size())
		{
			if (lastRestart < nitr)
			{
				while (lastRestart++ < nitr)
					flipsInRestart *= GEOM_FACTOR; // geometric decrease
			}
			...
			for (int i = 0; i < flipsInRestart; i++)
			{
				...
				r = rnd.getFloat();
				if ((xub - xlb) < BIGBIGM)
					newValue = floor(xlb + (1 + xub - xlb) * r);
				else if ((x[j] - xlb) < BIGM)
					newValue = xlb + (2 * BIGM - 1) * r;
				else if ((xub - x[j]) < BIGM)
					newValue = xub - (2 * BIGM - 1) * r;
				else
					newValue = x[j] + (2 * BIGM - 1) * r - BIGM;
		...
		else
		{
			...
			while (!changed)
			{
				// if (lessThan(threshold, 0.5))
				// 	std::cout << "THRESHOLD: " << threshold << std::endl;
				for (unsigned int i = 0; i < size; i++)
				{
					int j = binaries[i];
					r = rnd.getFloat();
					if (greaterThan(r, threshold))
					{
						x[j] = isNull(x[j], integralityEps) ? 1.0 : 0.0;
						++changed;
					}
				}
				threshold = std::max(0.0, threshold * 0.9); // to make sure it will eventually converge.
			}
			DOMINIQS_ASSERT(changed);
		}
		display.set("P", "**");
	...
	bool FeasibilityPump::pumpLoop(double &runningAlpha, int stage, double &dualBound, bool stopWithNoImprLimit)
	{
		// setup
		lastIntegerX.clear();
		multipleIntegerX.clear();
		...
		int maxItrNoImpr = (stage == 1) ? stage1NoImprIterLimit : stage2NoImprIterLimit;
		bool usedOrigFpNoRestart = false; // when using multiple reference points, if a cycle occurs use original FP instead of restart

		if (greaterThan(lpIterMult, 0.0))
		{
			lpIterLimit = int(rootLpIter * lpIterMult);
			lpIterLimit = std::max(lpIterLimit, 10);
		}
		bool lpfeasible = model->isSolutionFeasible(frac_x);

		// Disable PdLP warm start in stage 2 because of numerical issues
		if (stage == 2)
			model->intParam(IntParam::PdlpWarmStart, 0);

		while (!model->aborted() && ((nitr - oldIterCnt) < stageIterLimit) && ((nitr - oldIterCnt) < iterLimit))
		{
			// consoleInfo("{} - {} = {}/{}/{}", nitr, oldIterCnt, nitr - oldIterCnt, stageIterLimit, iterLimit);
			lpfeasible = model->isSolutionFeasible(frac_x);
			...
			if (lessEqualThan(timeLeft, 0.0))
				break;

			// display logger
			nitr++;
			display.resetIteration();
			if (display.needHeader(nitr))
				display.printHeader(std::cout);
			...
			if (lastIntegerX.size() && areSolutionsEqual(intSubset, integer_x, (*(lastIntegerX.begin())).second, integralityEps) && equal(runningAlpha, (*(lastIntegerX.begin())).first, alphaDist))
			{
				if (!pertCnt)
					firstPerturbation = nitr;
				// directly apply restart if PDLP stalls at an integer but not primal feasible solution
				if (!applyPdlpRestart)
					perturbe(integer_x, ignoreGenerals);
				else
					restart(integer_x, ignoreGenerals);
			}
			...
			if (model->isSolutionFeasible(integer_x))
			{
				thisAlpha = 0.0;
			...
			if (multipleIntegerX.size() > numIntegersObj)
				multipleIntegerX.resize(numIntegersObj);
			...
			if (harmonicWeights)
				scaleIntVec = harmonicVector(currNumPointsInObj);
			else
				scaleIntVec = exponDecayVector(intScaleFactor, currNumPointsInObj);
			...
			if (!aggregateInts)
			{
				for (int iter = 0; iter < currNumPointsInObj; iter++)
				{
					for (int j : binaries)
					{
						...
						{
							if (expObj)
								distCoef = scaleIntVec[iter] * 0.5 * exp(-0.5 * frac_x[j]);
							if (logisObj)
								distCoef = scaleIntVec[iter] * 0.1 * exp(-0.1 * frac_x[j]) / pow(1 + exp(-0.1 * frac_x[j]), 2);
							distObj[j] += distCoef;
						}
						else
						{
							if (expObj)
								distCoef = scaleIntVec[iter] * 0.5 * exp(-0.5 * (1 - frac_x[j]));
							if (logisObj)
								distCoef = scaleIntVec[iter] * 0.1 * exp(-0.1 * (1 - frac_x[j])) / pow(1 + exp(-0.1 * (1 - frac_x[j])), 2);
							distObj[j] -= distCoef;
						}
					}
				...
				if (stage > 1)
				{
					...
					for (int iter = 0; iter < currNumPointsInObj; iter++)
					{
						...
						for (int j : gintegers)
						{
							...
							{
								if (expObj)
									distCoef = scaleIntVec[iter] * 0.5 * exp(-0.5 * (frac_x[j] - lb[j]));
								if (logisObj)
									distCoef = scaleIntVec[iter] * 0.1 * exp(-0.1 * (frac_x[j] - lb[j])) / pow(1 + exp(-0.1 * (frac_x[j] - lb[j])), 2);
								distObj[j] += distCoef;
							}
							else if (equal(itrInt->second[j], ub[j], integralityEps))
							{
								if (expObj)
									distCoef = scaleIntVec[iter] * 0.5 * exp(-0.5 * (ub[j] - frac_x[j]));
								if (logisObj)
									distCoef = scaleIntVec[iter] * 0.1 * exp(-0.1 * (ub[j] - frac_x[j])) / pow(1 + exp(-0.1 * (ub[j] - frac_x[j])), 2);
								distObj[j] -= distCoef;
							}
							...
							else
							{
								...
								if (isSame)
								{
									if (iter == 0)
									{
										...
										if (expObj)
											distCoef = scaleIntVec[iter] * 0.5 * exp(-0.5 * fabs(frac_x[j] - itrInt->second[j]));
										if (logisObj)
											distCoef = scaleIntVec[iter] * 0.1 * exp(-0.1 * fabs(frac_x[j] - itrInt->second[j])) / pow(1 + exp(-0.1 * fabs(frac_x[j] - itrInt->second[j])), 2);
				...
				else
				{
					...
					if (!sameAsLast)
					{
						if (stage == 1)
						{
							...
							{
								if ((fabs(thisAlpha - itr->first) < alphaDist) && areSolutionsEqual(binaries, itr->second, new_integer, integralityEps))
									inCache = true;
								++itr;
							}
						}
						else
						{
							while ((itr != end) && !inCache)
							{
								if ((fabs(thisAlpha - itr->first) < alphaDist) && areSolutionsEqual(integers, itr->second, new_integer, integralityEps))
									inCache = true;
								++itr;
							}
						}
						...
						else if (model->isSolutionFeasible(new_integer))
						{
							thisAlpha = 0.0;
						}
					}
			...
			// solve LP
			if (lpIterLimit > 0)
				model->intParam(IntParam::IterLimit, lpIterLimit);

			timeLeft = std::max(std::min(timeLimit, pumpTimeLimit) - chrono.getElapsed(), 0.0);
			model->dblParam(DblParam::TimeLimit, timeLeft);

			// StopWatch tp2;
			// tp2.start();
			model->lpopt(reOptMethod, false, false);
			// if (greaterThan(tp2.getElapsed(), timeLeft))
			// 	std::cout << " * " << tp2.getElapsed() << " > " << timeLeft << std::endl;

			// consoleError("objValue={:.4f}", model->objval());
			lpWatch.stop();

			// if no solution is found because of timeLeft return false or problem infeasible.
			...
			if (primalFeas)
			{
				...
				while (pdlpTol > 1e-6 && !model->isSolutionFeasible(frac_x) && isSolutionInteger(intSubset, frac_x, integralityEps))
				{
					pdlpTol *= 0.1;
					...
					// StopWatch tp3;
					// tp3.start();
					model->lpopt(reOptMethod, true, false);
					// if (greaterThan(tp3.getElapsed(), timeLeft))
					// 	std::cout << " * " << tp3.getElapsed() << " > " << timeLeft << std::endl;
			...
			else
			{
				...
				consoleInfo("Reached FP time limit");
				break;
			}
			...
			// consoleError("{} {} {}", dist0, dist, numFrac);
			...
			if (lessViolatedIntegers)
			{
				...
				for (int j : integers)
					integer_afterProj[j] = integer_x[j];
				...
				if (bestObjIntegers)
				{
					...
					if (greaterThan(objOfInt, 0))
						itrIntegers->first = intViolation * objOfInt;
					else if (objOfInt == 0)
						itrIntegers->first = intViolation;
					else
						itrIntegers->first = (1.0 / (1.0 + intViolation)) * objOfInt;
				}
				else
					itrIntegers->first = intViolation;
			}
			if (lessDistanceIntegers)
			{
				std::list<AlphaVector>::iterator itrIntegers = multipleIntegerX.begin();
				if (bestObjIntegers)
					itrIntegers->first = dist * (dotProduct(&obj[0], &integer_x[0], n) + objOffset);
				else
					itrIntegers->first = dist;
			}
			...
			if (newStage3)
			{
				...
				if (stage3lessViolatedIntegers)
				{
					...
					if (stage3bestObjIntegers)
					{
						...
						if (greaterThan(objOfInt, 0))
							...
							scoreIntS3 = (1.0 / (1.0 + scoreIntS3)) * objOfInt;
					}
				}
				else
				{
				...
				if (p->size() > stage3IntegersObj)
					p->resize(stage3IntegersObj);
			}

			// consoleWarn("dist: {} | closestDist: {}", dist, closestDist);
			// std::vector<std::string> xNames;
			// model->colNames(xNames);
			// std::cout << "offset: " << objOffset << std::endl;
			// std::cout << "sol: " << std::endl;
			// for (int i : binaries)
			// {
			// 	// if (greaterThan(frac_x[i], 0))
			// 	std::cout << xNames[i] << " " << std::setprecision(15) << "frac: " << frac_x[i] << " int: " << integer_x[i] << std::endl;
			// }
			// save integer_x as best point if distance decreased and count number of itr without (1-imprThreshold)*100%% improvement
			if (lessThan(dist, closestDist))
			{
				// if (1-imprThreshold)*100% improvenent, set iterationsNoImpr = 0
				if (lessThan(dist / closestDist, imprThreshold))
					iterationsNoImpr = 0;
				closestDist = dist;
				closestPoint = integer_x;
				closestFrac = frac_x;
				// consoleInfo("melhorou");
			}
			else
				iterationsNoImpr++;
			...
			if (lpfeasible)
			{
				...
				if (dualBound == -static_cast<int>(model->objSense()) * INFBOUND)
					dualBound = origObj;
				if (model->objSense() == ObjSense::MIN)
					dualBound = std::min(dualBound, origObj);
				else
					dualBound = std::max(dualBound, origObj);
			}
			// display log
			...
			if (runningAlpha <= 1e-4)
				runningAlpha = 0.0;

			// break loop if too many iterations without 10% improvement
			if (stopWithNoImprLimit && (iterationsNoImpr >= maxItrNoImpr))
			{
				consoleInfo("Abort FP: {} iterations of stage {} without {:.2f}% improvement", maxItrNoImpr, stage, (1 - imprThreshold) * 100);
				break;
			}
		}

		// update the limit of iterations for the possible next calls of the feasibility (when within kernel pump).
		int niter_pump = (nitr - oldIterCnt);
		if (stage == 1)
		{
			stage1IterLimit = std::max(0, stage1IterLimit - niter_pump);
		}
		if (stage == 2)
		{
			stage2IterLimit = std::max(0, stage2IterLimit - niter_pump);
		}
		iterLimit = std::max(0, iterLimit - niter_pump);

		// consoleError(" Time Spent FP Stage {}: {}", stage, chrono.getElapsed());

		return (primalFeas && isSolutionInteger(intSubset, frac_x, integralityEps));
	}
	...
	bool FeasibilityPump::stage3()
	{
		...
		else if (!newStage3)
		{
			...
			for (int j : binaries)
				distObj[j] = (isNull(integer_x[j], integralityEps) ? 1.0 : -1.0);
			for (int j : gintegers)
			{
		...
		if (newStage3)
		{
			...
			if (stage3harmonicWeights)
				scaleIntVec = harmonicVector(currNumPointsInObj);
			else
				scaleIntVec = exponDecayVector(stage3ScaleFactor, currNumPointsInObj);
		...
		if (normalMIPStage3)
			model->objcoefs(colIndices.size(), &colIndices[0], &obj[0]);
		else if (newStage3 && (multirensStage3 || rensClosestDistStage3))
			model->objcoefs(colIndices.size(), &colIndices[0], &distObj[0]);
		else if (rensStage3 || multirensStage3 || rensClosestDistStage3)
			model->objcoefs(colIndices.size(), &colIndices[0], &obj[0]);
		else
			model->objcoefs(colIndices.size(), &colIndices[0], &distObj[0]);
	...
	bool FeasibilityPump::isInCache(double a, const std::vector<double> &x, bool ignoreGeneralIntegers)
	{
		...
		if (ignoreGeneralIntegers)
		{
			while ((itr != end) && !found)
			{
				if ((fabs(a - itr->first) < alphaDist) && areSolutionsEqual(binaries, itr->second, integer_x, integralityEps))
					found = true;
				++itr;
			}
		}
		else
		{
			while ((itr != end) && !found)
			{
				if ((fabs(a - itr->first) < alphaDist) && areSolutionsEqual(integers, itr->second, integer_x, integralityEps))
					found = true;
				++itr;
			}
		}
	...
	void FeasibilityPump::infeasibleSupport(const std::vector<double> &x, std::set<int> &supp, bool ignoreGeneralIntegers)
	{
		if (supp.empty())
		{
			...
			for (auto i : infeas)
			{
				...
				{
					int j = idx[k];
					if ((xType[j] == 'B') || ((xType[j] == 'I') && !ignoreGeneralIntegers))
						supp.insert(j);
				}
			}
		}
	...
	void FeasibilityPump::integerFromAC(std::vector<double> &x, double &bestgamma, double step)
	{
		...
		{
			for (int i = 0; i < n; i++)
				currentFrac[i] = (1.0 - gamma) * frac_x[i] + gamma * ac_x[i];
			frac2int->apply(currentFrac, currentInt);
			if (model->isSolutionFeasible(currentInt))
			{
				for (int i = 0; i < n; i++)
					x[i] = currentInt[i];
				bestgamma = gamma;
