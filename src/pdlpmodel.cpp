...
double PDLPModel::objval() const
{
	...
	for (int i = 0; i < ncols(); i++)
		objval += (coefs[i] * output.primal_solution(i));
	return objval;
}
...
void PDLPModel::reduced_costs(double *x, int first, int last) const
{
	// TODO!!!
...
void PDLPModel::dblParam(DblParam which, double value)
{
	...
	if (stageLP)
	{
		...
		case DblParam::TimeLimit:
			...
			if (solver != NULL)
				solver->set_time_limit((int)value * 1000);
