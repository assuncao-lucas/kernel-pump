...
void VarLowerBoundProp::propagate()
{
	...
	{
		if (domain.varType(xIdx) == 'I')
			pendingLb = ceilEps(pendingLb);
...
PropagatorPtr VarBoundFactory::analyze(Domain &d, const Constraint *c)
{
	if ((c->row.size() != 2) || (c->sense == 'E') || (c->sense == 'R'))
		return nullptr;
	...
	numCreated++;
	if (sense == 'L')
		return std::make_shared<VarUpperBoundProp>(d, c->name, xIdx, yIdx, yCoef, rhs);
