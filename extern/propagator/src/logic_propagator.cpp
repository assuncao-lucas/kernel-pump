	...
	: Propagator(d), anteIdx(xIdx), consIdx(yIdx)
{
	...
	{
		if (isNull(domain.varLb(anteIdx)))
			antecedent = FALSE_IDX;
		else
			antecedent = TRUE_IDX;
	}
	if (domain.isVarFixed(consIdx))
	{
		if (isNull(domain.varLb(consIdx)))
			consequent = FALSE_IDX;
		else
			consequent = TRUE_IDX;
	}
...
void EquivProp::propagate()
{
	...
	if (state == CSTATE_UNKNOWN)
	{
		...
		{
			if (isNull(domain.varLb(firstIdx)))
				domain.fixBinDown(secondIdx);
			else
				domain.fixBinUp(secondIdx);
		}
	}
	if (state == CSTATE_UNKNOWN)
	{
		...
		{
			if (isNull(domain.varLb(secondIdx)))
				domain.fixBinDown(firstIdx);
			else
				domain.fixBinUp(firstIdx);
		}
	}
}
...
void EquivProp::updateState()
{
	...
	{
		if (equal(domain.varLb(firstIdx), domain.varLb(secondIdx)))
			state = CSTATE_ENTAILED;
		else
			state = CSTATE_INFEAS;
	}
	else
		state = CSTATE_UNKNOWN;
}
...
PropagatorPtr LogicFactory::analyze(Domain &d, const Constraint *c)
{
	if (c->sense == 'R')
		return nullptr;
	...
	if (isImplies)
	{
		...
		if (equal(minCoef, -1.0) && equal(maxCoef, 1.0))
		{
			...
			{
				if (coef[0] > 0)
					return std::make_shared<ImpliesProp>(d, c->name, idx[0], idx[1]);
				else
					return std::make_shared<ImpliesProp>(d, c->name, idx[1], idx[0]);
			}
			else if (c->sense == 'E')
				return std::make_shared<EquivProp>(d, c->name, idx[0], idx[1]);
		}
	}
	return nullptr;
}
