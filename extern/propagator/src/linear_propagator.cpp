...
public:
	...
	void fixedUp()
	{
		if (prop.getState() != CSTATE_UNKNOWN)
			return;
...
LinearProp::LinearProp(Domain &d, const Constraint *c) : Propagator(d)
{
	DOMINIQS_ASSERT(c);
	name = c->name;
	switch (c->sense)
	...
	for (unsigned int k = 0; k < rowSize; k++)
	{
		...
		if (a > 0.0)
		{
			...
			{
				if (lessThan(domain.varUb(j), INFBOUND))
					maxAct += (domain.varUb(j) * a);
				else
					maxActInfCnt++;
				if (greaterThan(domain.varLb(j), -INFBOUND))
					minAct += (domain.varLb(j) * a);
				else
					minActInfCnt++;
				posIdx.push_back(j);
				posCoef.push_back(a);
		...
		else // a < 0.0
		{
			...
			{
				if (lessThan(domain.varUb(j), INFBOUND))
					minAct += (domain.varUb(j) * a);
				else
					minActInfCnt++;
				if (greaterThan(domain.varLb(j), -INFBOUND))
					maxAct += (domain.varLb(j) * a);
				else
					maxActInfCnt++;
				negIdx.push_back(j);
				negCoef.push_back(a);
...
void LinearProp::propagate()
{
	...
	if (maxActDelta < 0.0)
	{
		for (k = 0; k < lastPosBin; k++)
		{
			if (domain.isVarFixed(posBinIdx[k]))
				continue;
			if (maxActDelta < posBinCoef[k])
			{
				maxActDelta = posBinCoef[k];
			}
		}
		for (k = 0; k < lastNegBin; k++)
		{
			if (domain.isVarFixed(negBinIdx[k]))
				continue;
			if (maxActDelta < -negBinCoef[k])
			{
				maxActDelta = -negBinCoef[k];
			}
		}
		for (k = 0; k < lastPos; k++)
		{
			if (domain.isVarFixed(posIdx[k]))
				continue;
			double alpha = posCoef[k] * (domain.varUb(posIdx[k]) - domain.varLb(posIdx[k]));
			if (maxActDelta < alpha)
			{
				maxActDelta = alpha;
			}
		}
	...
	if (lessEqualThan(maxActDelta, std::min(slack, surplus)))
		return;			//< cannot deduce anything!
		...
		if (minActInfCnt == 1)
		{
			if (minActInfIdx == -1)
			{
				for (k = 0; k < lastPos; k++)
				{
					...
					if (domain.isVarFixed(j))
						continue;
					if (lessEqualThan(domain.varLb(j), -INFBOUND))
					{
						minActInfIdx = j;
						minActInfCoef = posCoef[k];
						break;
					}
				}
			}
			if (minActInfIdx == -1)
			{
				for (k = 0; k < lastNeg; k++)
				{
					...
					if (domain.isVarFixed(j))
						continue;
					if (greaterEqualThan(domain.varUb(j), INFBOUND))
					{
						minActInfIdx = j;
						minActInfCoef = negCoef[k];
						break;
					}
				}
			}
			if (minActInfIdx == -1)
				throw std::runtime_error("minActInfIdx to -1");
			if (minActInfCoef > 0.0)
			{
				newB = beta / minActInfCoef;
				if (domain.varType(minActInfIdx) != 'C')
					newB = floorEps(newB);
		...
		if (maxActInfCnt == 1)
		{
			...
			if (maxActInfIdx == -1)
			{
				for (k = 0; k < lastNeg; k++)
				{
					...
					{
						maxActInfIdx = j;
						maxActInfCoef = negCoef[k];
						break;
					}
				}
			}
			if (maxActInfIdx == -1)
				throw std::runtime_error("maxActInfIdx to -1");
			if (maxActInfCoef > 0.0)
			{
				newB = -beta / maxActInfCoef;
				if (domain.varType(maxActInfIdx) != 'C')
					newB = ceilEps(newB);
...
PropagatorPtr LinearFactory::analyze(Domain &d, const Constraint *c)
{
	numCreated++;
...
CardinalityProp::CardinalityProp(Domain &d, const Constraint *c) : Propagator(d)
{
	DOMINIQS_ASSERT(c);
	name = c->name;
...
{
	for (int i : idx)
		advisors.push_back(std::make_shared<CardinalityAdvisor>(*this, i));
}
...
PropagatorPtr CardinalityFactory::analyze(Domain &d, const Constraint *c)
{
	bool isCard = true;
	const int *idx = c->row.idx();
	const double *coef = c->row.coef();
	...
	{
		if (d.varType(idx[k]) != 'B')
			isCard = false;
		if (different(coef[k], 1.0))
			isCard = false;
	}
	if (isCard)
	{
...
KnapsackProp::KnapsackProp(Domain &d, const Constraint *c) : Propagator(d)
{
	DOMINIQS_ASSERT(c);
	name = c->name;
	// assuming data is correct
...
PropagatorPtr KnapsackFactory::analyze(Domain &d, const Constraint *c)
{
	bool isKp = true;
	const int *idx = c->row.idx();
	const double *coef = c->row.coef();
	...
	{
		if (isNegative(coef[k]))
			isKp = false;
		if (isNegative(d.varLb(idx[k])))
			isKp = false;
		if (!lessThan(d.varUb(idx[k]), INFBOUND))
			isKp = false;
	}
	if (isKp)
	{
