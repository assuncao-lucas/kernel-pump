...
namespace dominiqs
{
	...
	{
		auto c = std::find_if(columns.begin(), columns.end(), [&name](const ColumnMap::value_type &itr)
							  { return (itr.second->name == name); });
		columns.erase(c);
	}
	...
	void IterationDisplay::printIteration(std::ostream &out)
	{
		...
		for (auto &c : columns)
		{
			...
			if (col->visible)
			{
				if (current.find(col->name) != current.end())
					out << current[col->name];
				else
					out << col->formatHeader("");
			}
		}
		out << std::endl;
#endif //< SILENT_EXEC
