...
class PDLPModel : public MIPModelI
{

public:
	PDLPModel();
	PDLPModel(SCIP *scip, bool _ownProb = false);
	~PDLPModel() override;
	std::unique_ptr<PDLPModel> clone() const { return std::unique_ptr<PDLPModel>(this->clone_impl()); }
	/* Read/Write */
	...
	std::unique_ptr<PDLPModel> presolvedModel() { return std::unique_ptr<PDLPModel>(this->presolvedmodel_impl()); }
...
private:
	PDLPModel *clone_impl() const override;
	PDLPModel *presolvedmodel_impl() override;
	void copyLpi(SCIP_LPI **targetlpi, SCIP_LPI *srclpi);
	SCIP_STAGE getProbStage();
	void setPdlpTolerance(double tol, double decreaseFactor);
	double getPdlpTolerance();
	int numGeneralInts();
...
private:
	...
	int numColsPresolved;
	int numRowsPresolved;
	bool verbosity;

	// For PDLP
	operations_research::pdlp::QuadraticProgram *qp;
	std::vector<MPVariable *> variables;
	MPSolver *solver;
	operations_research::pdlp::TerminationReason result_status;
	operations_research::pdlp::SolverResult output;
	std::optional<operations_research::pdlp::PrimalAndDualSolution> initial_solution;
	operations_research::pdlp::PrimalDualHybridGradientParams params;
	double pdlpTol;
	double pdlpTolDecreaseFactor;
	bool pdlpWarmStart;
